...
public class VacancyControllerTest {
    ...
    @Test
    public void whenUpdateVacancyWithIdEqualOneThenReturnSavedVacancy() throws IOException {
    ...
    @Test
    public void whenUpdateVacancyThenThrowException() {
        Vacancy newVac = new Vacancy(1, "title1", "desc1",
                now(), true, 1, 2);
        ...
        Model model = new ConcurrentModel();
        String view = vacancyController.update(newVac, testFile, model);
        var actualExceptionMessage = model.getAttribute("message");
        assertThat(view).isEqualTo("errors/404");
        assertThat(expectedException.getMessage()).isEqualTo(actualExceptionMessage);
    }

    @Test
    public void whenDeleteVacancyThenReturnTrue() {
        Vacancy vacancy = new Vacancy(1, "title1", "desc1",
                now(), true, 1, 2);
        when(vacancyService.deleteById(vacancy.getId())).thenReturn(true);
        Model model = new ConcurrentModel();
        String view = vacancyController.delete(model, vacancy.getId());
        assertThat(view).isEqualTo("redirect:/vacancies");
    }

    @Test
    public void whenDeleteVacancyWithThenThrowException() {
        when(vacancyService.deleteById(any(Integer.class))).thenReturn(false);
        String expectedException = new RuntimeException("Вакансия с указанным идентификатором не найдена.").getMessage();
        Model model = new ConcurrentModel();
        String view = vacancyController.delete(model, 0);
        String actualExceptionMessage = model.getAttribute("message").toString();
        assertThat(view).isEqualTo("errors/404");
